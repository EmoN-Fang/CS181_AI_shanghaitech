...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        tmpFactors = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for eliVar in eliminationOrder:
            fac_nt_join, joined_fac = joinFactorsByVariable(tmpFactors, eliVar)
            if len(joined_fac.unconditionedVariables()) > 1:
                joined_fac = eliminate(joined_fac, eliVar)
                fac_nt_join.append(joined_fac)
            tmpFactors = fac_nt_join
        result_fac = joinFactors(fac_nt_join)
        result_fac = normalize(result_fac)
        return result_fac
