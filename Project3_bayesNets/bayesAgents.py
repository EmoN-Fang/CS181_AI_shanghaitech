...
def fillObsCPT(bayesNet, gameState):
    ...
    def check_pos(hpos):
        if hpos == bottomLeftPos: return BOTTOM_LEFT_VAL
        elif hpos == topLeftPos: return TOP_LEFT_VAL
        elif hpos == bottomRightPos: return BOTTOM_RIGHT_VAL
        elif hpos == topRightPos: return TOP_RIGHT_VAL

    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            obsVarFactor = bn.Factor([obsVar], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], bayesNet.variableDomainsDict())
            for ass in obsVarFactor.getAllPossibleAssignmentDicts():
                if ass[obsVar] == RED_OBS_VAL:
                    if ass[FOOD_HOUSE_VAR] == check_pos(housePos):
                        obsVarFactor.setProbability(ass, PROB_FOOD_RED)
                    elif ass[GHOST_HOUSE_VAR] == check_pos(housePos):
                        obsVarFactor.setProbability(ass, PROB_GHOST_RED)
                    else:
                        obsVarFactor.setProbability(ass, 0)
                elif ass[obsVar] == BLUE_OBS_VAL: 
                    if ass[FOOD_HOUSE_VAR] == check_pos(housePos):
                        obsVarFactor.setProbability(ass, 1 - PROB_FOOD_RED)
                    elif ass[GHOST_HOUSE_VAR] == check_pos(housePos):
                        obsVarFactor.setProbability(ass, 1 - PROB_GHOST_RED)
                    else:
                        obsVarFactor.setProbability(ass, 0)
                elif ass[obsVar] == NO_OBS_VAL:
                    if ass[FOOD_HOUSE_VAR] == check_pos(housePos) or ass[GHOST_HOUSE_VAR] == check_pos(housePos):
                        obsVarFactor.setProbability(ass, 0)
                    else:
                        obsVarFactor.setProbability(ass, 1)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    queryVars = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
    max_prob = 0
    for ass in queryVars.getAllPossibleAssignmentDicts():
        new_prob = queryVars.getProbability(ass)
